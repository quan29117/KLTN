...
def extract_hr_features(hr_signal, fs):
...
def get_multi(h5_file, base_fs=15.5):
    meta_info = {}
    if 'meta' in h5_file and 'fs' in h5_file['meta']:
        fs_group = h5_file['meta']['fs']
        for sig_name in fs_group.keys():
            current_fs = fs_group[sig_name][0]
            k = current_fs / base_fs
            meta_info[sig_name] = {'fs': current_fs, 'K': k}
            
    return meta_info

def extract_features(map_csv_path, h5_dir, output_path, window_sec=60):
    print(f"\n{'='*60}")
        ...
        with h5py.File(h5_file, 'r') as f:
            ...
            for _, row in drive_windows.iterrows():
                ...
                try:
                    ...
                    if 'EMG' in signals and 'EMG' in meta_map:
                        K = meta_map['EMG']['K']
                        fs = meta_map['EMG']['fs']
                        seg = signals['EMG'][int(s_base*K) : int(e_base*K)]
                        feat_dict.update(extract_emg_features(seg, fs))
                        
                    if 'HR' in signals and 'HR' in meta_map:
                        K = meta_map['HR']['K']
                        fs = meta_map['HR']['fs']
                        seg = signals['HR'][int(s_base*K) : int(e_base*K)]
                        feat_dict.update(extract_hr_features(seg, fs))
...
def run():
    extract_features(map_csv_path="./data/label/window_map.csv",
                     ...
                     output_path="./data/extracted_features/ml_features_dataset.csv")
